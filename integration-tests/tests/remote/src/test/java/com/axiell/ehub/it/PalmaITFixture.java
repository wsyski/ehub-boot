...
public abstract class PalmaITFixture {
    ...
    protected void givenPalmaCheckoutTestActiveLoanResponse() {
        stubFor(post(urlEqualTo("/arena.pa.palma/loans")).withRequestBody(containing(":CheckOutTest xmlns"))
                .willReturn(aResponse().withBodyFile("CheckOutTestResponse_activeLoan.xml").withHeader("Content-Type", "text/xml").withStatus(HttpServletResponse.SC_OK)));
    }

    protected void givenPalmaCheckoutTestNewLoanResponse() {
        stubFor(post(urlEqualTo("/arena.pa.palma/loans")).withRequestBody(containing(":CheckOutTest xmlns"))
                .willReturn(aResponse().withBodyFile("CheckOutTestResponse_newLoan.xml").withHeader("Content-Type", "text/xml").withStatus(HttpServletResponse.SC_OK)));
    }
